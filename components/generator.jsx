...
function Preview({ title, generation }) {
  ...
  return <section className="tool-preview" aria-label={title}>
    <div className="preview-head"><h2>{title}</h2><span>Markdown</span></div>
    <p className="preview-hint">입력한 내용으로 지침을 만듭니다. 빈 항목은 기본 문구로 표시됩니다.</p>
    <p className="generation-status" role="status">{pending ? '입력 내용을 반영하고 있습니다…' : result?.error ? '생성하지 못했습니다.' : '최신 입력이 반영되었습니다.'}</p>
    ...
    <div className="tool-actions"><button type="button" className="primary-button" disabled={!ready} onClick={async () => { try { await navigator.clipboard.writeText(result.markdown); notify('복사했습니다. 개발 도구나 문서에 붙여넣으세요.'); } catch { notify('자동 복사를 사용할 수 없습니다. 미리보기 내용을 직접 선택해 복사하세요.'); } }}>복사하기</button><button type="button" className="secondary-button" disabled={!ready} onClick={() => {
    ...
    }}>.md 다운로드</button></div><p className="tool-feedback" role="status">{ready && feedback?.markdown === result.markdown ? feedback.message : ''}</p>
  </section>;
}
...
export function ProjectGenerator() {
  ...
  return <><div className="eyebrow">PROJECT GUIDELINES</div><h1 className="article-title">우리 프로젝트의 개발 기준<span className="lime-period">.</span></h1><p className="intro">환경과 규칙을 정리하고 프로젝트에 바로 사용할 지침서를 만드세요.</p><p className="tool-privacy">입력은 이 사이트의 서버에서 처리하며 저장하지 않습니다. 외부 AI로 보내지 않으며, 새로고침하면 초기화됩니다.</p>
