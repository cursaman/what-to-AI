...
export function ProjectGenerator() {
  ...
  return <><div className="eyebrow">개발 도구</div><h1 className="article-title">프로젝트 지침 생성</h1><p className="intro">4단계에서 정한 개발 도구와 규칙을 입력하면 AI에게 전달할 지침서를 만들 수 있습니다.</p><p className="tool-privacy">입력은 이 사이트의 서버에서 처리하며 저장하지 않습니다. 외부 AI로 보내지 않으며, 새로고침하면 초기화됩니다.</p>
    <div className="tool-layout"><form className="tool-form" onSubmit={e => e.preventDefault()}><h2>프로젝트 설정</h2><Field name="name" label="프로젝트 이름" value={project.name} onChange={update('name')} placeholder="예: what-to-AI" error={errors.name} /><Field name="stack" label="사용할 개발 도구 (기술 스택)" value={project.stack} onChange={update('stack')} placeholder="예: Next.js, React, PostgreSQL" error={errors.stack} /><div className="field-pair"><Select name="language" label="코드 작성 언어" value={project.language} options={languages} onChange={update('language')} error={errors.language} /><Select name="manager" label="개발 도구 설치 프로그램" value={project.manager} options={managers} onChange={update('manager')} error={errors.manager} /></div>
